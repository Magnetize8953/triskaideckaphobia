...
if (num == 1) {
    for (i = 0; i < 13; i++) {
        ...
        instance_create_layer(_x_position, _y_position, "Instances", obj_Card, { card_id: _card , rotation: 0, scale: 0.18});
    }
} else if (num == 2) {
	for (i = 0; i < 13; i++) {
        _card = ds_list_find_value(hand, i); // an integer corresponding to the card id
        ...
        instance_create_layer(_x_position, _y_position, "Instances", obj_Card, { card_id: _card, rotation: 270, scale:0.11});
    }
} 
else if (num == 3) {
	for (i = 0; i < 13; i++) {
        ...
        _y_position = i * (_card_sprite_width * 0.27) + (_card_sprite_width);
        instance_create_layer(_x_position, _y_position, "Instances", obj_Card, { card_id: _card, rotation: 90, scale:0.11});
    }
}
